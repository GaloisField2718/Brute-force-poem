...
class OpenRouterClient {
    ...
    apiKey;
    constructor() {
        this.apiKey = config_1.Config.OPENROUTER_API_KEY;
        if (!this.apiKey) {
            throw new Error('OPENROUTER_API_KEY is required');
        }
        this.client = axios_1.default.create({
            baseURL: config_1.Config.OPENROUTER_BASE_URL,
            timeout: 30000,
            headers: {
     ...
     */
    async scoreWords(blank, candidateWords) {
        const prompt = this.buildPrompt(blank, candidateWords);
        try {
            const response = await this.client.post('/chat/completions', {
                model: config_1.Config.OPENROUTER_MODEL,
                temperature: 0.2,
                max_tokens: 1000,
                messages: [
                    {
            ...
            logger_1.logger.debug('OpenRouter scoring complete', {
            ...
            logger_1.logger.error('OpenRouter API error', {
                error: error.message,
                position: blank.position,
     ...
     */
    buildPrompt(blank, candidateWords) {
...
Return ONLY the JSON array, nothing else.`;
    ...
    parseResponse(content) {
        try {
            ...
            logger_1.logger.error('Failed to parse OpenRouter response', {
                error: String(error),
     ...
     */
    async scoreMultiplePositions(blanks, candidatesPerPosition) {
        const results = new Map();
        // Process sequentially to avoid rate limits
        for (const blank of blanks) {
            const candidates = candidatesPerPosition.get(blank.position);
            if (!candidates || candidates.length === 0) {
                logger_1.logger.warn('No candidates for position', { position: blank.position });
                continue;
            }
    ...
    async testConnection() {
        try {
            ...
            logger_1.logger.error('OpenRouter connection test failed', { error: String(error) });
            return false;
        }
    }
}
